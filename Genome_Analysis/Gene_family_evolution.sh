...
makeblastdb -in All_55species.fasta -dbtype prot -out blastdb_for_cafe
blastp -num_threads 50 -db blastdb_for_cafe -query All_55species.fasta -outfmt 7 -seg yes > blastp_output.txt
grep -v "^#" blastp_output.txt | cut -f 1,2,11 > blastp_output.abc

###################Clustering sequences with MCL
export PATH=/home/anaconda3/bin:$PATH
mcxload -abc blastp_output.abc --stream-mirror --stream-neg-log10 -stream-tf 'ceil(200)' -o blastp_output.mci -write-tab blastp_output.tab
mcl blastp_output.mci -I 3
mcxdump -icl out.blastp_output.mci.I30 -tabr blastp_output.tab -o dump.blastp_output.mci.I30
